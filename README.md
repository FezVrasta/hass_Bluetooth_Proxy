...
**Fix:** `time.monotonic()` is used directly, stamping each advertisement as received "now" by HA. Using the phone's Unix timestamp for the conversion is unreliable because it requires the phone clock to be in sync with the HA server; any drift is interpreted as advertisement age, causing readings like "10 seconds ago" for a phone whose clock is 10 seconds behind. The maximum error introduced by ignoring the phone timestamp equals the batch interval (≤ 3 s for typical app settings), which is well within Bermuda's staleness thresholds.
